...
@dp.message_handler(lambda message: message.text.isalpha(), state=BotState.waiting_code_simple_course)
async def process_currency_code(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        if valute_code == 'RUB':
            await message.answer("Курс рубля к рублю всегда 1.0. Введите другой код валюты (например, USD):")
            return
        if check_valute(valute_code)==(False,False):
            await message.answer("Валюта не найдена. Введите корректный код валюты (например, USD):")
            return
...
@dp.message_handler(lambda message: message.text.isalpha(), state=BotState.waiting_1_valute_curse)
...
@dp.message_handler(lambda message: message.text.isalpha(), state=BotState.waiting_2_valute_curse)
...
async def convertor(message: types.Message):
    await message.answer("Введите количество валюты для расчета:")
...
# Обработчик ввода первой валюты
@dp.message_handler(lambda message: message.text.isalpha(), state=BotState.waiting_1_valute_converter)
...
# Обработчик ввода второй валюты
@dp.message_handler(lambda message: message.text.isalpha(), state=BotState.waiting_2_valute_converter)
...
@dp.message_handler(lambda message: message.text.isalpha(), state=BotState.waiting_valute_graf)
async def process_currency_graph(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        data['currency_code'] = message.text.strip().upper()

    if data['currency_code'] == 'RUB':
        await message.answer("Курс рубля к рублю всегда 1.0. Не имеет смысла строить график. Введите другой код валюты (например, USD):")
        return
